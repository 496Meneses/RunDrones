...
void coordenadasIniciales(Game * game,int id){
	...
	char buf[512];
    								...
    								read(id, buf, 512);
										...
										game->drones = aux;	
										game->drones[0].position.x = atoi(buf);
										
								
									
										memset(buf, 0, 512);
    									read(id, buf, 512);
										game->drones[0].position.y = atoi(buf);
										...
										memset(buf, 0, 512);
    									read(id, buf, 512);
										game->drones[1].position.y = atoi(buf); //YO
										...
										game->site.x=  (float)atoi(buf); 
										
										
										memset(buf, 0, 512);
    									read(id, buf, 512);
										game->site.y = (float)atoi(buf);
...
void ProcessEvents(Game * game, int  id) {
		...
		FD_ZERO(&read_fds);
		FD_SET(id,&read_fds);
		if(select ((id + 1),&read_fds,0 ,0,&t)>=0){
        ...
        if (IsKeyDown(KEY_RIGHT)){
			 game->drones[1].position.x += 15.0f;
        ...
        if (IsKeyDown(KEY_UP)){ 
			game->drones[1].position.y -= 15.0f;
        ...
        if (IsKeyDown(KEY_DOWN)){ 
			...
			write(id, buf, 512);  
					...
					// LLEGADA
		if (game->state == GAMING){
        if ((game->drones[1].position.x >= (game->site.x - 60.0f))
		&&  (game->drones[1].position.x <=game->site.x + 60.0f)
		&& (game->drones[1].position.y >= (game->site.y - 60.0f))
		&&  (game->drones[1].position.y <=game->site.y+ 60.0f))
		{
